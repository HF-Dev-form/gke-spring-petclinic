...
build-with-dockerize:
	...
	@echo "pack build $(DOCKER_PREFIX)/spring-petclinic-k8s-$(SERVICE) --publish -e BP_MAVEN_BUILT_MODULE=$(MODULE) -e 'BP_MAVEN_BUILD_ARGUMENTS=clean package -DskipTests'\"" >> build.sh
	@chmod +x build.sh
	@./build.sh
	@rm build.sh
